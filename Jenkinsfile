    ...
    stages {        
       ...
       stage('dotnet test') {
          ...
          //  sh "dotnet tool install --global coverlet.console --version 1.4.1"
            ...
            sh "dotnet build-server shutdown"
            sh "dotnet sonarscanner begin /k:\"soanr_qube_qualitygates\" /d:sonar.host.url=http://localhost:9000 /d:sonar.cs.opencover.reportsPaths=\"calculation.tests\\coverage.opencover.xml\" /d:sonar.coverage.exclusions=\"**Tests*.cs\" "
            sh "dotnet build"
